...
void LiveLinkPanel::_bind_methods() {
  godot::ClassDB::bind_method(
    godot::D_METHOD("_on_start_button_pressed"),
...

  {
...

    {
...

      {
        ...
        _start_button->connect(
          "pressed", godot::Callable(this, "_on_start_button_pressed")
        );
...

    {
...

      {
        ...
        viewport->set_update_mode(
          godot::SubViewport::UpdateMode::UPDATE_ALWAYS
        );
...

        {
          ...
          camera->set_position(
            godot::Vector3(0, 0, 0.4)
          ); // back the camera away from the model
          ...
          godot::Ref<godot::ArrayMesh> face = res_loader->load(
            "res://addons/vdot/assets/tracking/live_link/face.res"
          );
          if (!face.is_valid()) {
            ...
            // NOTE: Due to how the files are copied over as part of the
            // plugin,
            //  setting the material and texture links via the editor results
            //  in them not coming over correctly - hence loading and set them
            //  manually.
            ...
            head_mat->set_texture(
              godot::BaseMaterial3D::TEXTURE_ALBEDO, head_tex
            );
...
void LiveLinkPanel::_process(double delta) {
  if (_server_active) { _server->poll(); }
}
...
void LiveLinkPanel::start_server() {
  godot::UtilityFunctions::print(
    "Starting LiveLinkFace server on: ", _server->get_port()
  );
...
void LiveLinkPanel::stop_server() {
  godot::UtilityFunctions::print(
    "Stopping LiveLinkFace server on: ", _server->get_port()
  );
...
void LiveLinkPanel::update_mesh(godot::Ref<LiveLinkClientData> const data) {
...
void LiveLinkPanel::_on_server_client_connected(
  godot::Ref<LiveLinkClient> const &client
) {}

void LiveLinkPanel::_on_server_client_disconnected(
  godot::Ref<LiveLinkClient> const &client
) {}

void LiveLinkPanel::_on_server_client_updated(
  godot::Ref<LiveLinkClient> const &client
) {
