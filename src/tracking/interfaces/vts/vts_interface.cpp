...
VTSInterface *VTSInterface::get_singleton() { return singleton; }

void VTSInterface::_bind_methods() {
  godot::ClassDB::bind_method(
    godot::D_METHOD("initialize"), &VTSInterface::initialize
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("uninitialize"), &VTSInterface::uninitialize
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("is_initialized"), &VTSInterface::is_initialized
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("register_tracker", "address", "port"),
    &VTSInterface::register_tracker
  );
}

VTSInterface::VTSInterface() { singleton = this; }

VTSInterface::~VTSInterface() { _trackers.clear(); }

bool VTSInterface::is_initialized() { return _initialized; }
...
godot::Ref<VTSFaceTracker> VTSInterface::register_tracker(
  godot::String const &address,
  uint16_t             port
) {
  ERR_FAIL_COND_V_MSG(
    _initialized == false, nullptr, "VTS Interface needs to be initialized."
  );
