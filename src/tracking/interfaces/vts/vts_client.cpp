...
godot::Error VTSData::decode_packet(godot::PackedByteArray const &packet) {
  ...
  ERR_FAIL_COND_V_MSG(
    ...
    err,
    godot::vformat(
      "Received invalid JSON from VTS server: %s", decoded->get_error_message()
    )
  );
  ...
  head_position                        = godot::Vector3(
    head_position_dict["x"], head_position_dict["y"], head_position_dict["z"]
  );

  godot::Dictionary head_rotation_dict = data.get("Rotation", _empty_vec3);
  head_rotation                        = godot::Vector3(
    head_rotation_dict["x"], head_rotation_dict["y"], head_rotation_dict["z"]
  );

  godot::Dictionary left_eye_rotation_dict = data.get("EyeLeft", _empty_vec3);
  eye_rotation_left                        = godot::Vector3(
    left_eye_rotation_dict["x"],
    left_eye_rotation_dict["y"],
    left_eye_rotation_dict["z"]
  );

  godot::Dictionary right_eye_rotation_dict = data.get(
    "EyeRight", _empty_vec3
  );
  eye_rotation_right = godot::Vector3(
    right_eye_rotation_dict["x"],
    right_eye_rotation_dict["y"],
...
VTSClient::VTSClient() { _data = godot::Ref<VTSData>(memnew(VTSData)); }

VTSClient::~VTSClient() = default;

godot::Error VTSClient::connect() {
  ERR_FAIL_COND_V_MSG(
    _client != nullptr, godot::FAILED, "Client is already running."
  );

  // First, build the request packet we send repeatedly to request new tracking
  // datga.
...
godot::Error VTSClient::receive() {
  // Send the packet to instruct the server we're still here and still want
  // data.
  auto err = _client->put_packet(_request_packet);
  ERR_FAIL_COND_V_MSG(err != godot::OK, err, "Failed to connect to host.");

  if (_client->get_available_packet_count() < 1) { return godot::OK; }

  ERR_FAIL_COND_V_MSG(
    _client->get_packet_error() != godot::OK,
    _client->get_packet_error(),
    "Packet error."
  );
  ...
  if (packet.size() == 0) { return godot::OK; }
