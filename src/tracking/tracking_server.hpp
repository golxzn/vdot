...
public:
  // NOTE: mirroring values from Godot's experimental XR interface for eventual
  // compatability.
  //  ref:
...
public:
  ...
  void add_interface(godot::Ref<TrackingInterface> const &p_interface);
  void remove_interface(godot::Ref<TrackingInterface> const &p_interface);
  int  get_interface_count() const;
  godot::Ref<TrackingInterface> get_interface(int p_index) const;
  godot::Ref<TrackingInterface> find_interface(
    godot::String const &p_name
  ) const;
  godot::TypedArray<godot::Dictionary> get_interfaces() const;

  void                add_tracker(godot::Ref<Tracker> const &p_tracker);
  void                remove_tracker(godot::Ref<Tracker> const &p_tracker);
  godot::Dictionary   get_trackers(TrackerType p_tracker_types);
  godot::Ref<Tracker> get_tracker(godot::StringName const &p_name) const;
};
