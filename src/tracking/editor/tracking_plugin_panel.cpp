...
void TrackingPluginPanel::_bind_methods() {
  godot::ClassDB::bind_method(
    godot::D_METHOD("_on_interface_added", "interface_name"),
    &TrackingPluginPanel::_on_interface_added
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("_on_interface_removed", "interface_name"),
  ...
  godot::ClassDB::bind_method(
    godot::D_METHOD("_on_interface_selected", "index"),
    &TrackingPluginPanel::_on_interface_selected
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("_on_tracker_selected", "index"),
...
TrackingPluginPanel::TrackingPluginPanel() {
  ...
  server->connect(
    "interface_added", godot::Callable(this, "_on_interface_added")
  );
  server->connect(
    "interface_removed", godot::Callable(this, "_on_interface_removed")
  );
  server->connect("tracker_added", godot::Callable(this, "_on_tracker_added"));
  server->connect(
    "tracker_removed", godot::Callable(this, "_on_tracker_removed")
  );
...

  {
...

    {
...

      {
        ...
        _interface_list->connect(
          "item_selected", godot::Callable(this, "_on_interface_selected")
        );
...

      {
        ...
        _tracker_list->connect(
          "item_selected", godot::Callable(this, "_on_tracker_selected")
        );
...
void TrackingPluginPanel::_update_tracker_list() {
  ...
  auto face_trackers = server->get_trackers(
    TrackingServer::TrackerType::TRACKER_FACE
  );
...
void TrackingPluginPanel::_on_interface_added(
  godot::StringName const &p_interface_name
) {
  auto idx = _interface_list->add_item(p_interface_name);
  _interface_list->set_item_metadata(idx, p_interface_name);
}

void TrackingPluginPanel::_on_interface_removed(
  godot::StringName const &p_interface_name
) {
...
void TrackingPluginPanel::_on_tracker_added(
...
void TrackingPluginPanel::_on_tracker_removed(
  godot::StringName const    &p_tracker_name,
  TrackingServer::TrackerType p_tracker_type
) {
...
void TrackingPluginPanel::_on_interface_selected(int p_index) {
  godot::StringName interface_name = _interface_list->get_item_metadata(
    p_index
  );
