...
#include "standards/unified_expressions.hpp"
#include "tracker.hpp"
...
protected:
  ...
  godot::Ref<InputParameter>
    blend_shape_parameters[UnifiedExpressions::BlendShape::FT_MAX] = {};

public:
  FaceTracker();
  ~FaceTracker() override;

  [[nodiscard]] float get_blend_shape(
    UnifiedExpressions::BlendShape blend_shape
  ) const;
  void set_blend_shape(
    UnifiedExpressions::BlendShape blend_shape,
    float                          weight
  );

  [[nodiscard]] godot::PackedFloat32Array get_blend_shapes() const;
  void set_blend_shapes(godot::PackedFloat32Array const &p_blend_shapes);

  godot::StringName get_blend_shape_name(
    UnifiedExpressions::BlendShape blend_shape
  );
  ...
  [[nodiscard]] float get_arkit_blend_shape_weight(
    ARKit::BlendShape blend_shape
  ) const;
};
