...
void TrackingServer::_bind_methods() {
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_interface", "index"), &TrackingServer::get_interface
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("find_interface", "name"), &TrackingServer::find_interface
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_interfaces"), &TrackingServer::get_interfaces
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("add_tracker", "tracker"), &TrackingServer::add_tracker
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("remove_tracker", "tracker"),
    &TrackingServer::remove_tracker
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_trackers", "tracker_types"),
    &TrackingServer::get_trackers
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_tracker", "tracker_name"),
    &TrackingServer::get_tracker
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("_process"), &TrackingServer::_process
  );
  ...
  ADD_SIGNAL(
    godot::MethodInfo(
      "tracker_added",
      godot::PropertyInfo(godot::Variant::STRING_NAME, "tracker_name"),
...
TrackingServer *TrackingServer::get_singleton() { return singleton; }

TrackingServer::TrackingServer() { singleton = this; }
...
void TrackingServer::add_interface(
  godot::Ref<TrackingInterface> const &p_interface
) {
...
void TrackingServer::remove_interface(
  godot::Ref<TrackingInterface> const &p_interface
) {
  ERR_FAIL_COND(p_interface.is_null());
  ...
  godot::UtilityFunctions::print(
    "Tracking: Removed interface \"" + p_interface->get_name() + "\""
  );
...
godot::Ref<TrackingInterface> TrackingServer::get_interface(
  int p_index
) const {
...
godot::Ref<TrackingInterface> TrackingServer::find_interface(
  godot::String const &p_name
) const {
  for (auto const &_interface : _interfaces) {
    if (_interface->get_name() == p_name) { return _interface; }
  }
...
void TrackingServer::add_tracker(godot::Ref<Tracker> const &p_tracker) {
  ...
  if (_trackers.has(tracker_name)) {
    if (_trackers[tracker_name] != p_tracker) {
      ...
      emit_signal(
        "tracker_updated", tracker_name, p_tracker->get_tracker_type()
      );
    }
  } else {
    _trackers[tracker_name] = p_tracker;
...
void TrackingServer::remove_tracker(godot::Ref<Tracker> const &p_tracker) {
  ERR_FAIL_COND(p_tracker.is_null());
...
godot::Dictionary TrackingServer::get_trackers(TrackerType p_tracker_types) {
  ...
  for (int i = 0; i < _trackers.keys().size(); i++) {
    ...
    if (
      tracker.is_valid() &&
      (tracker->get_tracker_type() & p_tracker_types) != 0
    ) {
...
godot::Ref<Tracker> TrackingServer::get_tracker(
