...
void FaceTracker::_bind_methods() {
  ...
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_blend_shape", "blend_shape"),
    &FaceTracker::get_blend_shape
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("set_blend_shape", "blend_shape", "weight"),
    &FaceTracker::set_blend_shape
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("get_blend_shape_name", "blend_shape"),
    &FaceTracker::get_blend_shape_name
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("get_arkit_blend_shape", "arkit_blend_shape"),
  ...
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_blend_shapes"), &FaceTracker::get_blend_shapes
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("set_blend_shapes", "weights"),
    &FaceTracker::set_blend_shapes
  );
  ADD_PROPERTY(
    godot::PropertyInfo(godot::Variant::PACKED_FLOAT32_ARRAY, "blend_shapes"),
    "set_blend_shapes",
    "get_blend_shapes"
  );
}
...
FaceTracker::FaceTracker() {
  ...
  parameter_context->context_id = godot::vformat(
    "/trackers/%s", get_tracker_name()
  );
  parameter_context->context_label = godot::vformat(
    "Facial Tracker %s", get_tracker_name()
  );
  parameter_context->context_description = godot::vformat(
    "Facial Tracker %s", get_tracker_name()
  );
  ...
  for (int i = 0; i < UnifiedExpressions::BlendShape::FT_MAX; i++) {
    ...
    param->parameter_id = godot::vformat(
      "/tracking/facial/%s/%s",
      get_tracker_name(),
      UnifiedExpressions::blend_shape_names[blend_shape]
    );
    param
      ->parameter_label = UnifiedExpressions::blend_shape_names[blend_shape];
  ...
  ERR_FAIL_COND_MSG(
    parameters == nullptr, "Expected parameter server to be initialized."
  );
...
float FaceTracker::get_blend_shape(
  UnifiedExpressions::BlendShape blend_shape
) const {
...
void FaceTracker::set_blend_shape(
  UnifiedExpressions::BlendShape blend_shape,
  float                          weight
) {
  // Fail if the blend shape index is out of range.
...
void FaceTracker::set_blend_shapes(
  godot::PackedFloat32Array const &p_blend_shapes
) {
  // Fail if the blend shape array is not the correct size.
  ERR_FAIL_COND(
    p_blend_shapes.size() != UnifiedExpressions::BlendShape::FT_MAX
  );
  ...
  for (int i = 0; i < UnifiedExpressions::BlendShape::FT_MAX; i++) {
    ...
    emit_signal(
      "blend_shape_updated", blend_shape, blend_shape_values[blend_shape]
    );
  }
...
godot::StringName FaceTracker::get_blend_shape_name(
  UnifiedExpressions::BlendShape blend_shape
) {
...
float FaceTracker::get_arkit_blend_shape_weight(
  ARKit::BlendShape blend_shape
) const {
  if (!UnifiedExpressions::arkit_to_unified.has(blend_shape)) { return 0.0f; }

  auto unified_shape = UnifiedExpressions::arkit_to_unified[blend_shape];

  // Fail if the blend shape index is out of range.
  ERR_FAIL_INDEX_V(
    unified_shape, UnifiedExpressions::BlendShape::FT_MAX, 0.0f
  );
