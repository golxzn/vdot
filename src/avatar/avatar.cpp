...
void Avatar::_bind_methods() {
  ...
  ADD_SIGNAL(
    godot::MethodInfo(
      ...
      godot::PropertyInfo(godot::Variant::DICTIONARY, "parameters")
    )
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("get_avatar_parameters"), &Avatar::get_avatar_parameters
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("set_avatar_parameters", "parameters"),
  ...
  ADD_PROPERTY(
    ...
    "set_avatar_parameters",
    "get_avatar_parameters"
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("_apply_parameter", "id", "value"),
    &Avatar::_apply_parameter
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("get_model"), &Avatar::get_model
  );
  ADD_PROPERTY(
    godot::PropertyInfo(
  ...
  godot::ClassDB::bind_method(
    godot::D_METHOD("_child_entered_tree", "node"),
    &Avatar::_child_entered_tree
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("_child_exiting_tree", "node"),
...
void Avatar::_notification(int p_what) {
  ...
  if (
    p_what == godot::Node::NOTIFICATION_PATH_RENAMED ||
...
void Avatar::_ready() {
  {
    ...
    // Parameter updates will be called often, and we don't need all of them to
    // sync.
    opts["transfer_mode"] =
      godot::MultiplayerPeer::TRANSFER_MODE_UNRELIABLE_ORDERED;
    // Shortcut to prevent calling apply twice, have it call the local peer
    // too.
    opts["call_local"] = true;
    opts["channel"]    = 0; // TODO: allocate a channel specifically for avatar
                            // parameter updates
...
void Avatar::_enter_tree() { _update_model_references(); }

void Avatar::_exit_tree() { _update_model_references(); }

void Avatar::_input(godot::Ref<godot::InputEvent> const &p_event) {
  ...
  if (mouse_button.is_valid()) {
    if (mouse_button->get_button_index() == godot::MOUSE_BUTTON_LEFT) {
      if (mouse_button->is_pressed()) {
        ...
        drag_offset    = get_global_position() -
                         mouse_button->get_global_position();
      } else {
        mouse_dragging = false;
      }
    } else if (
      mouse_button->get_button_index() == godot::MOUSE_BUTTON_WHEEL_UP
    ) {
      set_scale(godot::Vector2(get_scale().x + 0.02f, get_scale().y + 0.02f));
    } else if (
      mouse_button->get_button_index() == godot::MOUSE_BUTTON_WHEEL_DOWN
    ) {
      set_scale(godot::Vector2(get_scale().x - 0.02f, get_scale().y - 0.02f));
    }
  }
...
void Avatar::_process(double delta) { _process_parameters(delta); }
...
void Avatar::_process_parameters(double delta) {
  ...
  for (int i = 0; i < parameters.size(); i++) {
    ...
    if (!parameter.is_valid()) { continue; }

    godot::Ref<AvatarParameterEval>
      eval     = parameter_values[parameter->get_parameter_id()];
    ...
    // TODO: apply smoothing (use smoothstep?), use eval to store smoothing
    // data.
...
Model *Avatar::get_model() const { return model; }
...
godot::PackedStringArray Avatar::_get_configuration_warnings() const {
  ...
  if (model == nullptr) { // children.size() == 0 ||
    warnings.push_back(
      "Avatar is missing a model. Add a node descended from Model to this "
      "node's children."
    );
...
void Avatar::set_avatar_parameters(
  godot::TypedArray<AvatarParameter> const &p_parameters
) {
...
void Avatar::_apply_parameter(godot::StringName const &p_id, float p_value) {
...
godot::Ref<AvatarBundle> Avatar::pack_bundle() const {
  ...
  if (model != nullptr) {
    ...
    ERR_FAIL_COND_V_MSG(
      model_bundle.is_null(), {}, "Failed to pack model for avatar bundle."
    );

    bundle->add_model(model_bundle);
  } else {
    WARN_PRINT(
      "No model is loaded for the Avatar. Packing the avatar to a bundle will "
...
void Avatar::_update_model_references() {
  ...
  if (model == nullptr) {
    ...
    for (int i = 0; i < children.size(); i++) {
      ...
      if (
        godot::ClassDB::is_parent_class(
          Model::get_class_static(), child->get_class_static()
        )
      ) {
  ...
  if (model == nullptr) {
    ...
    if (is_inside_tree()) { set_texture(model->get_texture()); }
  }
