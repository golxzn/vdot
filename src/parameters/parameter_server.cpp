...
void ParameterServer::_bind_methods() {
  ...
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_parameter_context", "context_id"),
    &ParameterServer::get_parameter_context
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("list_context_ids"), &ParameterServer::list_context_ids
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("add_parameter_context", "parameter_context"),
    &ParameterServer::add_parameter_context
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("remove_parameter_context", "context_id"),
    &ParameterServer::remove_parameter_context
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("_context_parameter_added"),
    &ParameterServer::_context_parameter_added
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("_context_parameter_removed"),
    &ParameterServer::_context_parameter_removed
  );
}

ParameterServer *ParameterServer::get_singleton() { return singleton; }

ParameterServer::ParameterServer() { singleton = this; }

godot::HashMap<godot::StringName, godot::Ref<ParameterContext>>
ParameterServer::get_parameter_contexts() const {
...
godot::TypedArray<godot::StringName> ParameterServer::
  list_context_ids() const {
  godot::TypedArray<godot::StringName> context_ids;

  for (godot::KeyValue<godot::StringName, godot::Ref<ParameterContext>> const
...
void ParameterServer::add_parameter_context(
  ...
  p_parameter_context->connect(
    ParameterContext::SIGNAL_PARAMETER_ADDED,
...
void ParameterServer::remove_parameter_context(
  ...
  p_parameter_context->connect(
    ParameterContext::SIGNAL_PARAMETER_REMOVED,
...
void ParameterServer::_context_parameter_added(
  godot::StringName const          &p_context_id,
  ...
  ERR_FAIL_COND_MSG(
    !parameter_contexts.has(p_context_id),
    godot::vformat(
      "Got signal for context '%s' which isn't registered.", p_context_id
    )
  );

  emit_signal(
    SIGNAL_CONTEXT_PARAMETER_ADDED,
    parameter_contexts[p_context_id],
    p_parameter
  );
}

void ParameterServer::_context_parameter_removed(
  godot::StringName const          &p_context_id,
  godot::Ref<InputParameter> const &p_parameter
) {
  ERR_FAIL_COND_MSG(
