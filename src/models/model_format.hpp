...
public:
   ...
   */
  [[nodiscard]] virtual godot::PackedStringArray
  get_recognized_extensions() const {
    return {};
  }

  /**
   * Alternative matches that's checked if get_recognized_extensions doesn't
   * pass.
   * @return if the format can handle the specified path
   */
  [[nodiscard]] virtual bool can_handle_path(
   ...
   */
  virtual Model *load_from_path(godot::String const &p_path) const {
   ...
   */
  [[nodiscard]] virtual godot::PackedStringArray
  get_supported_formats() const {
    ERR_FAIL_V_MSG({}, "get_supported_formats not implemented");
  }

  /**
   * Returns the object class to use for loading models supported by this
   * format.
   ...
   */
  virtual Model *create_from_bundle(
