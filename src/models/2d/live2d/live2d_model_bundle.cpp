...
void Live2DModelBundle::_bind_methods() {
  godot::ClassDB::bind_method(
    godot::D_METHOD("set_model3_path", "path"),
    &Live2DModelBundle::set_model3_path
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_model3_path"), &Live2DModelBundle::get_model3_path
  );
  ADD_PROPERTY(
    godot::PropertyInfo(godot::Variant::STRING, "model3_path"),
    "set_model3_path",
    "get_model3_path"
  );
}
...
void Live2DModelBundle::set_model3_path(godot::String const &p_path) {
  model3_path = p_path;
}

godot::Error Live2DModelBundle::pack_from_model3(godot::String const &p_path) {
  ...
  // Record the path to the model3 file in the bundle for reference when
  // loading.
  ...
  err = add_file(
    p_path.get_base_dir().path_join(gd_filename), p_path.get_base_dir()
  );
  ...
  if (strcmp(model_setting->GetPhysicsFileName(), "") != 0) {
    ...
    err = add_file(
      p_path.get_base_dir().path_join(phys_filename), p_path.get_base_dir()
    );
    ERR_FAIL_COND_V_MSG(
      err != godot::OK, err, "Failed to pack .physics3.json"
    );
  }
  ...
  if (strcmp(model_setting->GetPoseFileName(), "") != 0) {
    ...
    err = add_file(
      p_path.get_base_dir().path_join(pose_filename), p_path.get_base_dir()
    );
  ...
  if (strcmp(model_setting->GetDisplayInfoFileName(), "") != 0) {
    ...
    err = add_file(
      p_path.get_base_dir().path_join(info_filename), p_path.get_base_dir()
    );
    ERR_FAIL_COND_V_MSG(
      err != godot::OK, err, "Failed to pack display info file"
    );
  }
  ...
  if (strcmp(model_setting->GetUserDataFile(), "") != 0) {
    ...
    err = add_file(
      p_path.get_base_dir().path_join(userdata_filename), p_path.get_base_dir()
    );
    ERR_FAIL_COND_V_MSG(
      err != godot::OK, err, "Failed to pack userdata filename"
    );
  }

  for (int i = 0; i < model_setting->GetTextureCount(); i++) {
    if (strcmp(model_setting->GetTextureFileName(i), "") == 0) { continue; }

    godot::String tex_filename;
    tex_filename.parse_utf8(model_setting->GetTextureFileName(i));
    err = add_file(
      p_path.get_base_dir().path_join(tex_filename), p_path.get_base_dir()
    );
    ERR_FAIL_COND_V_MSG(
      err != godot::OK,
      err,
  ...
  for (int i = 0; i < model_setting->GetExpressionCount(); i++) {
    if (strcmp(model_setting->GetExpressionFileName(i), "") == 0) { continue; }

    godot::String exp_filename;
    exp_filename.parse_utf8(model_setting->GetExpressionFileName(i));
    err = add_file(
      p_path.get_base_dir().path_join(exp_filename), p_path.get_base_dir()
    );
    ERR_FAIL_COND_V_MSG(
      err != godot::OK,
      err,
  ...
  for (int i = 0; i < model_setting->GetMotionGroupCount(); i++) {
    if (strcmp(model_setting->GetMotionGroupName(i), "") == 0) { continue; }
    ...
    for (int x = 0; x < model_setting->GetMotionCount(group_name); x++) {
      ...
      motion_filename.parse_utf8(
        model_setting->GetMotionFileName(group_name, x)
      );
      err = add_file(
        p_path.get_base_dir().path_join(motion_filename), p_path.get_base_dir()
      );
      ERR_FAIL_COND_V_MSG(
        err != godot::OK,
        err,
