...
protected:
  static void _bind_methods() {
    godot::ClassDB::bind_method(
      godot::D_METHOD("set_target", "target"), &CubismEffectHitArea::set_target
    );
    godot::ClassDB::bind_method(
      godot::D_METHOD("get_target"), &CubismEffectHitArea::get_target
    );

    godot::ClassDB::bind_method(
      godot::D_METHOD("get_detail", "model", "id"),
      &CubismEffectHitArea::get_detail
    );

    godot::ClassDB::bind_method(
      godot::D_METHOD("set_monitoring", "value"),
      &CubismEffectHitArea::set_monitoring
    );
    godot::ClassDB::bind_method(
      godot::D_METHOD("get_monitoring"), &CubismEffectHitArea::get_monitoring
    );
    ADD_PROPERTY(
      godot::PropertyInfo(godot::Variant::BOOL, "monitoring"),
      "set_monitoring",
      "get_monitoring"
    );
...
private:
  ...
  static godot::Rect2 build_rect2(
    godot::Ref<godot::ArrayMesh> const &ref_ary_mesh
  ) {
    godot::Array              ary       = ref_ary_mesh->surface_get_arrays(0);
    godot::PackedVector2Array ary_vtx   = ary[godot::Mesh::ARRAY_VERTEX];
    int64_t const             vtx_count = ary_vtx.size();
  ...
  [[nodiscard]] godot::Array check_collide_triangle(
    ...
    if (
      ref_ary_mesh->surface_get_primitive_type(0) !=
    ...
    for (int64_t i = 0; i < ary_idx.size(); i += 3) {
      int32_t const ia        = ary_idx[static_cast<int>(i) + 0];
      int32_t const ib        = ary_idx[static_cast<int>(i) + 1];
      int32_t const ic        = ary_idx[static_cast<int>(i) + 2];

      godot::Vector2 const ab = ary_vtx[ib] - ary_vtx[ia];
      godot::Vector2 const bc = ary_vtx[ic] - ary_vtx[ib];
      godot::Vector2 const ca = ary_vtx[ia] - ary_vtx[ic];
      godot::Vector2 const ax = this->_target - ary_vtx[ia];
      godot::Vector2 const bx = this->_target - ary_vtx[ib];
      godot::Vector2 const cx = this->_target - ary_vtx[ic];
...
public:
  ...
  godot::Vector2 get_target() const { return this->_target; }

  godot::Dictionary get_detail(
    CubismModel const   *model,
    godot::String const &id
  ) {
    if (!this->_initialized) { return {}; }
    if (model == nullptr) { return {}; }
    if (!model->is_initialized()) { return {}; }

    godot::Dictionary const dict_mesh = model->get_meshes();
    if (dict_mesh.is_empty()) { return {}; }

    godot::Ref<godot::ArrayMesh> ref_ary_mesh = dict_mesh[id];
    if (ref_ary_mesh.is_null()) { return {}; }
    if (ref_ary_mesh->surface_get_array_index_len(0) < 3) { return {}; }
    ...
    if (rect.has_point(this->_target)) {
      ...
      if (vertices.size() == 3) { dict_result["vertices"] = vertices; }
    }

    return dict_result;
  }

  void set_monitoring(bool const &value) {
    this->_dict_monitoring.clear();
    this->_monitoring = value;
  }

  [[nodiscard]] bool get_monitoring() const { return this->_monitoring; }
  ...
  void _cubism_process(CubismModelProxy *model, float const delta) override {
    ...
    for (int64_t i = 0; i < ary.size(); i++) {
      godot::String const id = static_cast<godot::Dictionary>(
                                 ary[static_cast<int>(i)]
      )
                                 .get("id", godot::String());
      if (!dict_mesh.has(id)) { continue; }
      ...
      if (this->_target_update && check_rect.has_point(this->_target)) {
        if (this->_monitoring) {
          if (!static_cast<bool>(this->_dict_monitoring.get(id, false))) {
            this->emit_signal(
              SIGNAL_EFFECT_HIT_AREA_ENTERED, model->_owner_viewport, id
            );
          }
        } else {
          this->emit_signal(
        ...
        if (static_cast<bool>(this->_dict_monitoring.get(id, false))) {
          this->emit_signal(
            SIGNAL_EFFECT_HIT_AREA_EXITED, model->_owner_viewport, id
          );
        }
  ...
  void _cubism_epilogue(CubismModelProxy *model, float const delta) override {
    this->_target_update = false;
  }
};
