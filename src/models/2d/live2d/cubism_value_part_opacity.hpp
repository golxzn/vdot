...
private:
  void setup(Csm::CubismModel *model, Csm::csmInt32 index) override {
    ...
    this->id                                  = godot::String(
      Live2D::Cubism::Core::csmGetPartIds(csm_model)[index]
    );
    this->_value = Live2D::Cubism::Core::csmGetPartOpacities(csm_model)[index];
    this->raw_value = &(
      Live2D::Cubism::Core::csmGetPartOpacities(csm_model)[index]
    );
    this->changed = false;
  }
...
public:
  CubismPartOpacity()
    : CubismValueAbs(CubismValueAbs::ValueType::PART_OPACITY) {}
};
