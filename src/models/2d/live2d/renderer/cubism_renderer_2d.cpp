...
godot::PackedVector2Array make_PackedArrayVector2(
  Live2D::Cubism::Core::csmVector2 const *ptr,
  int32_t const                          &size
);
godot::PackedVector2Array make_PackedArrayVector3(
  Live2D::Cubism::Core::csmVector2 const *ptr,
  int32_t const                          &size,
  Csm::csmFloat32 const                  &ppunit,
  godot::Vector2 const                   &vct_adjust
);
godot::Vector4 make_vector4(Live2D::Cubism::Core::csmVector4 const &src_vec4);
...
godot::Ref<godot::ShaderMaterial> CubismRenderer2D::make_ShaderMaterial(
  ...
  if (model->GetDrawableMaskCounts()[index] == 0) {
    switch (model->GetDrawableBlendMode(index)) {
      case CubismBlendMode_Additive      : e = CUBISM_SHADER_NORM_ADD; break;
      case CubismBlendMode_Normal        : e = CUBISM_SHADER_NORM_MIX; break;
      case CubismBlendMode_Multiplicative: e = CUBISM_SHADER_NORM_MUL; break;
      default                            : e = CUBISM_SHADER_NORM_MIX; break;
    }
  } else {
    if (!model->GetDrawableInvertedMask(index)) {
      switch (model->GetDrawableBlendMode(index)) {
        case CubismBlendMode_Additive      : e = CUBISM_SHADER_MASK_ADD; break;
        case CubismBlendMode_Normal        : e = CUBISM_SHADER_MASK_MIX; break;
        case CubismBlendMode_Multiplicative: e = CUBISM_SHADER_MASK_MUL; break;
        default                            : e = CUBISM_SHADER_MASK_MIX; break;
      }
    } else {
      switch (model->GetDrawableBlendMode(index)) {
        case CubismBlendMode_Additive: e = CUBISM_SHADER_MASK_ADD_INV; break;
        case CubismBlendMode_Normal  : e = CUBISM_SHADER_MASK_MIX_INV; break;
        case CubismBlendMode_Multiplicative:
          e = CUBISM_SHADER_MASK_MUL_INV;
          break;
        default: e = CUBISM_SHADER_MASK_MIX_INV; break;
      }
    }
  }

  godot::Ref<godot::Shader> shader = res._owner_viewport->get_shader(e);
  if (shader.is_null()) { shader = res.get_shader(e); }

  mat->set_shader(shader);

  CubismTextureColor const color_base = this->GetModelColorWithOpacity(
    model->GetDrawableOpacity(index)
  );

  mat->set_shader_parameter(
    "color_base",
    godot::Vector4(color_base.R, color_base.G, color_base.B, color_base.A)
  );
  mat->set_shader_parameter(
    "color_screen", make_vector4(model->GetDrawableScreenColor(index))
  );
  mat->set_shader_parameter(
    "color_multiply", make_vector4(model->GetDrawableMultiplyColor(index))
  );
  mat->set_shader_parameter("channel", godot::Vector4(0.0, 0.0, 0.0, 1.0));
  mat->set_shader_parameter(
    "tex_main", res.ary_texture[model->GetDrawableTextureIndex(index)]
  );

  return mat;
}

void CubismRenderer2D::make_ArrayMesh_prepare(
  Csm::CubismModel const *model,
  ::CubismRenderer       &res
) {
  godot::Vector2 const vct_size   = this->get_size(model);
  godot::Vector2 const vct_origin = this->get_origin(model);
  float const          ppunit     = this->get_ppunit(model);

  res.vct_canvas_size             = res._owner_viewport->get_size();

  if (
    res._owner_viewport->mask_viewport_size.x > 0 &&
  ...
  res.CALCULATED_ORIGIN_C = (godot::Vector2(res.vct_canvas_size) *
                             vct_origin) /
                            vct_size;
  res.CALCULATED_ORIGIN_M = (godot::Vector2(res.vct_mask_size) * vct_origin) /
                            vct_size;
  res.RATIO = float(res.vct_mask_size.x) / float(res.vct_canvas_size.x);

  if (res._owner_viewport->auto_scale) {
    float fdstC = godot::MIN(
      static_cast<float>(res.vct_canvas_size.x),
      static_cast<float>(res.vct_canvas_size.y)
    );
    float fdstM = godot::MIN(
      static_cast<float>(res.vct_mask_size.x),
...
godot::Ref<godot::ArrayMesh> CubismRenderer2D::make_ArrayMesh(
  Csm::CubismModel const *model,
  Csm::csmInt32 const     index,
  bool const              maskmode,
  ::CubismRenderer const &res
) const {
  godot::Vector2 const adjust_pos = res.adjust_pos;
  godot::Array         ary;
...
Csm::csmInt32 CubismRenderer2D::calc_viewport_count() const {
  Live2D::Cubism::Framework::CubismModel const *model = this->GetModel();
  Csm::csmInt32                                 count = 0;

  for (Csm::csmInt32 index = 0; index < model->GetDrawableCount(); index++) {
    if (model->GetDrawableMaskCounts()[index] > 0) { count++; }
  }
...
godot::Vector2 CubismRenderer2D::get_size(
  Csm::CubismModel const *model
) const {
  ...
  Live2D::Cubism::Core::csmReadCanvasInfo(
    model->GetModel(), &vct_size, &vct_origin, &ppunit
  );

  return { vct_size.X, vct_size.Y };
}

godot::Vector2 CubismRenderer2D::get_origin(
  Csm::CubismModel const *model
) const {
  Live2D::Cubism::Core::csmVector2 vct_size;
  Live2D::Cubism::Core::csmVector2 vct_origin;
...
float CubismRenderer2D::get_ppunit(Csm::CubismModel const *model) const {
  Live2D::Cubism::Core::csmVector2 vct_size;
  Live2D::Cubism::Core::csmVector2 vct_origin;
  Csm::csmFloat32                  ppunit;
...
void CubismRenderer2D::update_mask(
  ...
  godot::Ref<godot::Shader> shader = res._owner_viewport->get_shader(
    CUBISM_SHADER_MASK
  );
  if (shader.is_null()) { shader = res.get_shader(CUBISM_SHADER_MASK); }
  ...
  for (Csm::csmInt32 m_index = 0;
       m_index < model->GetDrawableMaskCounts()[index];
       m_index++) {
...
void CubismRenderer2D::update(::CubismRenderer &res) {
  Live2D::Cubism::Framework::CubismModel const *model = this->GetModel();
  Csm::csmInt32 const *renderOrder = model->GetDrawableRenderOrders();
  ...
  for (Csm::csmInt32 index = 0; index < model->GetDrawableCount(); index++) {
    ...
    godot::MeshInstance2D            *node = res.request_mesh_instance();
    godot::Ref<godot::ShaderMaterial> mat  = this->make_ShaderMaterial(
      model, index, res
    );

    Live2D::Cubism::Framework::CubismIdHandle handle = model->GetDrawableId(
      index
    );
    ...
    if (model->GetDrawableMaskCounts()[index] > 0) {
      ...
      viewport->set_clear_mode(
        godot::SubViewport::ClearMode::CLEAR_MODE_ALWAYS
      );
      ...
      viewport->set_use_own_world_3d(
        false
      ); // SUBVIEWPORT_USE_OWN_WORLD_3D_FLAG
      // Memory leak when set_transparent_background is true(* every time &
      // window minimize) https://github.com/godotengine/godot/issues/89651
      ...
      mat->set_shader_parameter(
        "canvas_size", godot::Vector2(res.vct_canvas_size)
      );
      mat->set_shader_parameter(
        "mask_size", godot::Vector2(res.vct_mask_size)
      );
    ...
    godot::Ref<godot::ArrayMesh> m = this->make_ArrayMesh(
      model, index, false, res
    );
...
void CubismRenderer2D::Initialize(
...
godot::PackedInt32Array make_PackedArrayInt32(
  ...
  ary.resize(size);
  for (int i = 0; i < size; i++) { ary.set(i, ptr[i]); }
  return ary;
}
...
godot::PackedVector2Array make_PackedArrayVector3(
  ...
  for (int i = 0; i < size; i++) {
    ary.set(
      i,
      (godot::Vector2(ptr[i].X, static_cast<real_t>(ptr[i].Y * -1.0)) *
       ppunit) +
        vct_adjust
    );
  }
