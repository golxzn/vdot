...
CubismModelProxy::CubismModelProxy(
  ...
  , _moc3_file_format_version(
      CubismModel::moc3FileFormatVersion::CSM_MOC_VERSION_UNKNOWN
    )
  , _model_pathname("")
...
CubismModelProxy::~CubismModelProxy() { this->clear(); }

bool CubismModelProxy::model_load(godot::String const &model_pathname) {
  this->_model_pathname         = model_pathname;
  this->_updating               = true;
  this->_initialized            = false;
  this->_model_setting          = nullptr;

  godot::PackedByteArray buffer = godot::FileAccess::get_file_as_bytes(
    this->_model_pathname
  );
  if (buffer.size() == 0) { return false; }

  this->clear();

  this->_model_setting = CSM_NEW
    Live2D::Cubism::Framework::CubismModelSettingJson(
      buffer.ptr(),
  ...
  if (strcmp(this->_model_setting->GetModelFileName(), "") == 0) {
    ...
    godot::String moc3_pathname = this->_model_pathname.get_base_dir()
                                    .path_join(gd_filename);
    ...
    Live2D::Cubism::Core::csmVersion const
      version = Live2D::Cubism::Core::csmGetMocVersion(
        buffer.ptr(),
        static_cast<Live2D::Cubism::Framework::csmSizeInt>(buffer.size())
      );
  ...
  // EyeBlink(Parameters)
  {
    Csm::csmInt32 param_count = this->_model_setting
                                  ->GetEyeBlinkParameterCount();
    for (Csm::csmInt32 i = 0; i < param_count; ++i) {
      this->_list_eye_blink.PushBack(
        this->_model_setting->GetEyeBlinkParameterId(i)
      );
    }
  }
  ...
  // LipSync(Parameters)
  {
    ...
    for (Csm::csmInt32 i = 0; i < param_count; ++i) {
      this->_list_lipsync.PushBack(
        this->_model_setting->GetLipSyncParameterId(i)
      );
    }
  }
  ...
  // Motion
  if (this->_owner_viewport->enable_load_motions) { this->motion_load(); }
  ...
  // The process is almost the same as the CubismModelProxy::update_node()
  // function.
  {
    auto *renderer = this->GetRenderer<CubismRenderer2D>();

    // Update Adjust Parameter(s)
    this->_renderer_resource.adjust_scale = this->_owner_viewport
                                              ->adjust_scale;
    this->_renderer_resource.adjust_pos   = this->_owner_viewport->adjust_pos;

    this->_renderer_resource.pro_proc(
      renderer->calc_viewport_count(), renderer->calc_mesh_instance_count()
    );
...
void CubismModelProxy::model_load_resource() {
  ...
  for (Live2D::Cubism::Framework::csmInt32 index = 0;
       index < this->_model_setting->GetTextureCount();
       index++) {
    ...
    godot::String texture_pathname   = this->_model_pathname.get_base_dir()
                                         .path_join(gd_filename);

    godot::Ref<godot::Texture2D> tex = res_loader->load(texture_pathname);
    if (!tex.is_valid()) { // Texture will invalid if the texture loader cannot
                           // handle it.
      ...
      godot::UtilityFunctions::push_warning(
        "Live2D model texture was loaded as an ImageTexture: %s",
        texture_pathname
      );
      ...
      if (!tex.is_valid()) {
        godot::UtilityFunctions::push_error(
          "Failed to load Live2D model texture: %s", texture_pathname
        );
        continue;
      }
    }
...
void CubismModelProxy::pro_update(float const delta) {
  ...
  if (
    this->_owner_viewport->parameter_mode ==
    CubismModel::ParameterMode::FULL_PARAMETER
  ) {
...
void CubismModelProxy::efx_update(float const delta) {
  if (!this->IsInitialized()) { return; }
  if (this->_model_setting == nullptr) { return; }
  if (this->_model == nullptr) { return; }
...
void CubismModelProxy::epi_update(float const delta) {
  if (!this->IsInitialized()) { return; }
  if (this->_model_setting == nullptr) { return; }
  if (this->_model == nullptr) { return; }
...
void CubismModelProxy::clear() {
...

  {
    ...
    for (Live2D::Cubism::Framework::csmMap<
           ...
           Live2D::Cubism::Framework::CubismExpressionMotion *>::const_iterator
           i = this->_map_expression.Begin();
         i != this->_map_expression.End();
         i++) {
...
Live2D::Cubism::Framework::CubismMotionQueueEntryHandle CubismModelProxy::
  motion_start(
    char const   *group,
    int32_t const no,
  ...
  Live2D::Cubism::Framework::csmString
    name = Live2D::Cubism::Framework::Utils::CubismString::GetFormatedString(
      "%s_%d", group, no
    );
...
void CubismModelProxy::MotionEventFired(
  Live2D::Cubism::Framework::csmString const &eventValue
) {
...
void CubismModelProxy::expression_load() {
  ...
  for (Live2D::Cubism::Framework::csmInt32 i = 0;
       i < this->_model_setting->GetExpressionCount();
       i++) {
    Live2D::Cubism::Framework::csmString name = this->_model_setting
                                                  ->GetExpressionName(i);

    godot::String gd_filename;
    gd_filename.parse_utf8(this->_model_setting->GetExpressionFileName(i));
    godot::String expression_pathname = this->_model_pathname.get_base_dir()
                                          .path_join(gd_filename);
    ...
    auto *motion =
      dynamic_cast<Live2D::Cubism::Framework::CubismExpressionMotion *>(
        this->LoadExpression(
          buffer.ptr(),
          static_cast<Live2D::Cubism::Framework::csmSizeInt>(buffer.size()),
    ...
    if (this->_map_expression[name] != nullptr) {
      Live2D::Cubism::Framework::ACubismMotion::Delete(
        this->_map_expression[name]
      );
...
void CubismModelProxy::physics_load() {
  ...
  godot::String physics_pathname = this->_model_pathname.get_base_dir()
                                     .path_join(gd_filename);
...
void CubismModelProxy::pose_load() {
  ...
  godot::String pose_pathname = this->_model_pathname.get_base_dir().path_join(
    gd_filename
  );
...
void CubismModelProxy::userdata_load() {
  ...
  godot::String userdata_pathname = this->_model_pathname.get_base_dir()
                                      .path_join(gd_filename);
...
void CubismModelProxy::motion_load() {
  ...
  for (Live2D::Cubism::Framework::csmInt32 ig = 0;
       ig < this->_model_setting->GetMotionGroupCount();
       ig++) {
    // PreloadMotionGroup(group);
    Live2D::Cubism::Framework::csmChar const
      *group = this->_model_setting->GetMotionGroupName(ig);
    Live2D::Cubism::Framework::csmInt32 const
      motion_count = this->_model_setting->GetMotionCount(group);
    ...
    for (Live2D::Cubism::Framework::csmInt32 im = 0; im < motion_count; im++) {
      Live2D::Cubism::Framework::csmString name =
        Live2D::Cubism::Framework::Utils::CubismString::GetFormatedString(
          "%s_%d", group, im
        );

      godot::String gd_filename;
      gd_filename.parse_utf8(
        this->_model_setting->GetMotionFileName(group, im)
      );
      godot::String motion_pathname = this->_model_pathname.get_base_dir()
                                        .path_join(gd_filename);

      godot::PackedByteArray buffer = godot::FileAccess::get_file_as_bytes(
        motion_pathname
      );
      auto *motion = dynamic_cast<Live2D::Cubism::Framework::CubismMotion *>(
        this->LoadMotion(
          buffer.ptr(),
          static_cast<Live2D::Cubism::Framework::csmSizeInt>(buffer.size()),
          name.GetRawString()
      ...
      if (fade_time_sec >= 0.0f) { motion->SetFadeInTime(fade_time_sec); }

      fade_time_sec = this->_model_setting->GetMotionFadeOutTimeValue(
        group, im
      );
      if (fade_time_sec >= 0.0f) { motion->SetFadeOutTime(fade_time_sec); }
      static_cast<Live2D::Cubism::Framework::CubismMotion *>(motion)
        ->SetEffectIds(this->_list_eye_blink, this->_list_lipsync);

      if (this->_map_motion[name] != nullptr) {
        Live2D::Cubism::Framework::ACubismMotion::Delete(
          this->_map_motion[name]
        );
...
void CubismModelProxy::effect_batch(
  float const       delta,
  const EFFECT_CALL efx_call
) {
  for (Csm::csmVector<CubismEffect *>::iterator i =
         this->_owner_viewport->_list_cubism_effect.Begin();
       i != this->_owner_viewport->_list_cubism_effect.End();
