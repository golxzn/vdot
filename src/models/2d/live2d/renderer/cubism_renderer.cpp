...
CubismRenderer::CubismRenderer(
  ...
  this->ary_shader[CUBISM_SHADER_NORM_ADD] = res_loader->load(
    "res://addons/vdot/assets/live2d/shader/2d_cubism_norm_add.gdshader"
  );
  this->ary_shader[CUBISM_SHADER_NORM_MIX] = res_loader->load(
    "res://addons/vdot/assets/live2d/shader/2d_cubism_norm_mix.gdshader"
  );
  this->ary_shader[CUBISM_SHADER_NORM_MUL] = res_loader->load(
    "res://addons/vdot/assets/live2d/shader/2d_cubism_norm_mul.gdshader"
  );

  this->ary_shader[CUBISM_SHADER_MASK] = res_loader->load(
    "res://addons/vdot/assets/live2d/shader/2d_cubism_mask.gdshader"
  );

  this->ary_shader[CUBISM_SHADER_MASK_ADD] = res_loader->load(
    "res://addons/vdot/assets/live2d/shader/2d_cubism_mask_add.gdshader"
  );
  ...
  this->ary_shader[CUBISM_SHADER_MASK_MIX] = res_loader->load(
    "res://addons/vdot/assets/live2d/shader/2d_cubism_mask_mix.gdshader"
  );
  ...
  this->ary_shader[CUBISM_SHADER_MASK_MUL] = res_loader->load(
    "res://addons/vdot/assets/live2d/shader/2d_cubism_mask_mul.gdshader"
  );
...
godot::SubViewport *CubismRenderer::request_viewport() {
  Csm::csmInt32 const counter = this->sub_viewport_counter++;

  if (counter < this->ary_sub_viewport.size()) {
    return godot::Object::cast_to<godot::SubViewport>(
      this->ary_sub_viewport[counter]
    );
  } else {
...
godot::MeshInstance2D *CubismRenderer::request_mesh_instance() {
  Csm::csmInt32 const counter = this->mesh_instance_counter++;

  if (counter < this->ary_mesh_instance.size()) {
    return godot::Object::cast_to<godot::MeshInstance2D>(
      this->ary_mesh_instance[counter]
    );
  } else {
...
void CubismRenderer::pro_proc(
  Csm::csmInt32 const viewport_count,
  Csm::csmInt32 const mesh_instance_count
) {
...
void CubismRenderer::dispose_node(bool const node_release) {
...
void _recurisive_dispose_node(
  ...
  for (Csm::csmInt32 i = 0; i < ary_node.size(); i++) {
    ...
    if (node != nullptr) {
      ...
      if (node_release) { node->queue_free(); }
    }
  }
}
