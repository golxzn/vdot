...
void Model::_bind_methods() {
  godot::ClassDB::bind_method(
    godot::D_METHOD("get_model_parameters"), &Model::get_model_parameters
  );
  ADD_PROPERTY(
    godot::PropertyInfo(
      godot::Variant::ARRAY,
  ...
  godot::ClassDB::bind_method(
    godot::D_METHOD("_on_property_updated"), &Model::_on_property_updated
  );
}

void Model::_add_model_parameter(
  godot::Ref<ModelParameter> const &p_parameter
) {
  parameters.append(p_parameter);

  p_parameter->connect(
    "value_updated", godot::Callable(this, "_on_property_updated")
  );
}
...
bool Model::_property_can_revert(godot::StringName const &p_name) const {
  for (int i = 0; i < parameters.size(); i++) {
    godot::Ref<ModelParameter> parameter = parameters[i];

    if (p_name == parameter->get_parameter_id()) { return true; }
  }

  return false;
}

bool Model::_property_get_revert(
  godot::StringName const &p_name,
