...
LipsyncServer *LipsyncServer::get_singleton() { return singleton; }
...
LipsyncServer::LipsyncServer() {
  ...
  voice_volume_param->parameter_description =
    "How loud the voice of the current microphone is.";
  param_context->add_input_parameter(voice_volume_param);

  ParameterServer *server = ParameterServer::get_singleton();
  ERR_FAIL_COND_MSG(
    server == nullptr, "Expected ParameterServer to be initialized."
  );
