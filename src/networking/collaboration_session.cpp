...
void CollaborationSession::_bind_methods() {
  ...
  godot::ClassDB::bind_method(
    godot::D_METHOD("start_server"), &CollaborationSession::start_server
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("connect_to_server", "address", "port"),
    &CollaborationSession::connect_to_server
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("_client_connected_to_server"),
    &CollaborationSession::_client_connected_to_server
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("_client_connection_failed"),
    &CollaborationSession::_client_connection_failed
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("_client_server_disconnected"),
    &CollaborationSession::_client_server_disconnected
  );

  godot::ClassDB::bind_method(
    godot::D_METHOD("_peer_connected", "peer_id"),
    &CollaborationSession::_peer_connected
  );
  godot::ClassDB::bind_method(
    godot::D_METHOD("_peer_disconnected", "peer_id"),
...
void CollaborationSession::_ready() {
  ...
  get_multiplayer()->connect(
    "peer_connected", godot::Callable(this, "_peer_connected")
  );
  get_multiplayer()->connect(
    "peer_disconnected", godot::Callable(this, "_peer_disconnected")
  );

  get_multiplayer()->connect(
    "connected_to_server", godot::Callable(this, "_client_connected_to_server")
  );
  get_multiplayer()->connect(
    "connection_failed", godot::Callable(this, "_client_connection_failed")
  );
  get_multiplayer()->connect(
    "server_disconnected", godot::Callable(this, "_client_server_disconnected")
  );
}
...
godot::Error CollaborationSession::connect_to_server(
  godot::String const &p_address,
  int                  p_port
) {
  auto err = local_peer->enet_peer->create_client(p_address, p_port);
  ERR_FAIL_COND_V_MSG(
    err != godot::OK, err, "Failed to attempt server connection."
  );
